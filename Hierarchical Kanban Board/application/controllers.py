...
def home():
    ...
    yettodo = Task.query.filter_by(status='Yet To Do', user_id=g.user.id).all()
    currentlydoing = Task.query.filter_by(status='Currently Doing', user_id=g.user.id).all()
    completed = Task.query.filter_by(status='Completed', user_id=g.user.id).all()
    task_dict = {
        'Yet To Do': yettodo,
        'Currently Doing': currentlydoing,
        'Completed': completed
    }
